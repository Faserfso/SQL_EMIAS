...
FROM
    ...
    LEFT OUTER JOIN etd2.lpu lpu ON lpu.id = raur.lpu_id
    ...
    LEFT OUTER JOIN etd2.account_lpu_blocking_status albs ON albs.account_id = raur.login_id
                                                                      AND albs.lpu_id = raur.lpu_id
--where raur.lpu_id in (
                        --10095553, 112,192,10000284,10000285,10000454,10000251,10000308 --gp12
                        --10000247106,236,10000290 --dgp110
--)
                        
--AND a.status='ACTIVE'
--AND albs.status='ACTIVE'
